...
const Profile = () => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:8080/v1/user?uid=${userId}`
        );
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await axios.get(
          `http://localhost:8080/v1/orders?uid=${userId}`
        );
        setOrders(
          response.data.sort(
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      setIsLoading(true);
      await axios.delete(
        `http://localhost:8080/v1/order?id=${orderId}&uid=${userId}`
      );
      ...
      >
        ...
        <div className="mt-3">
          ...
          <div className="flex justify-between py-2 border-b last:border-b-0">
            <div>
              <p>Product #{order.product_id?.slice(0, 8)}</p>
