...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      ...
      try {
        const cartIdResponse = await axios.post(
        ...
        const cartResponse = await axios.get(
          `http://localhost:8080/v1/cart?cart_id=${cartIdResponse.data.cart_id}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
            },
        ...
        const transformedItems = cartResponse.data.map((item) => ({
          id: item.product_id,
  ...
  const handleQuantityChange = async (itemId, newQuantity) => {
    ...
    try {
      await axios.put(
        `http://localhost:8080/v1/cart/${itemId}`,
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const orderPromises = cartItems.map(async (item) => {
        ...
        return axios.post("http://localhost:8080/v1/order", orderData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
          },
        });
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="container mx-auto py-12 px-4 sm:px-6 lg:px-8 pt-24">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl font-bold text-gray-900 mb-5 mt-5 text-center">
            Your Shopping Cart
          </h2>
