...
const ProductDetail = () => {
    ...
    const confirmOrder = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:8080/v1/order', orderData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('auth_token')}`
                }
            });
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex gap-2 mt-2">
                                {[0.5, 1, 2, 3].map((weight) => (
                                    <button
                                        key={weight}
                                        className={`px-4 py-2 rounded-lg ${orderDetails.pounds === weight ? 'bg-pink-600 text-white' : 'bg-gray-200 text-gray-800'}`}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg max-w-md w-full">
                        ...
                        <div className="space-y-3">
                            ...
                            <p><span className="font-semibold">Weight:</span> {orderDetails.pounds} lb</p>
                            <p><span className="font-semibold">Quantity:</span> {orderDetails.quantity}</p>
                            <p><span className="font-semibold">Price:</span> ${(product.price * orderDetails.pounds).toFixed(2)}</p>
                            <p><span className="font-semibold">Delivery Date:</span> {orderDetails.delivery_date}</p>
                            <p><span className="font-semibold">Delivery Time:</span> {orderDetails.delivery_time}</p>
                            <p>
                                <span className="font-semibold">Delivery Address:</span>
                                {userAddress ? ` ${userAddress}` : ' Same as in your profile'}
                            </p>
                            {orderDetails.message && (
                                <p><span className="font-semibold">Message:</span> {orderDetails.message}</p>
                            )}
                        </div>
